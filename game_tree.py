"""Library for representing the game tree."""
...
Action = recordclass(
    'Action',
    [
        'before',  # The state the action was taken in.
        ...
        'for_maximizer'
    ])  # A boolean, True if this action is for the maximizer


class State(object):
  """An object representing a state in the game tree."""

  def __init__(self, states, game, model, rollout_policy,
               opponent_rollout_policy):
    """Constructor for a State.

    Args:
      states: A global map from state keys to all visited states in the game
        tree.
      game: The shared Game object.
      model: The shared Model object.
      rollout_policy: A function with signature Dict[int, Action]->int used to
        select the action key of the action to take during rollouts.
      opponent_rollout_policy: A function with signature Dict[int, Action]->int
        used to the action key of the action to take for the opponent during
        rollouts. None means self-play (the rollout policy is used for both
        players).
    """
    # Needs access to the game so that it can use game dynamics
    self.game = game
    # Needs access to the model to evaluate future states.
    self.model = model
    # A map from string ids to all the known states so that we don't create
    # duplicate States.
    self.states = states
    # A numpy array that uniquely identifies this state.
    self.id = game.state()
    # The model's score of the state.
    self.prior = model.value(self.id)
    # Add this state to the global list of states
    self.states[game.state_key()] = self
    # Function that governs how to select a move during rollouts.
    self.rollout_policy = rollout_policy
    # Function that governs how the opponent selects a move during rollouts
    # (or None for self play).
    ...
    if not self.is_terminal:
      ...
      self.actions = {
          self.game.action_index(move): Action(
              before=self,
              after={},
              ...
              for_maximizer=self.maximizer_to_act)
          for move in game.available_actions()
      }

  def backup(self, outcome, current_depth, rollout_actions):
    """Backs up the results from the bottom of a rollout.

    Updates statistics for the visited nodes, and undoes the moves.

    Args:
      outcome: A float, outcome to propagate back through the game tree for each
        action's average_value property.
      current_depth: An int, the current depth of the backup in the game tree
        i.e. how much farther the backup should go.
      rollout_actions: A list of the Actions taken during the rollout_policy so
        that the backup can be properly applied.
    """
    if current_depth != 0:
      ...
      action.average_value += (
          outcome - action.average_value) / action.visit_count
      self.game.undo_action()
  ...
  def rollout(self, rollout_depth, current_depth, rollout_actions):
    ...
    After the rollout is complete, backs up the results to all actions taken
    during the rollout.
    ...
    """
    if rollout_depth == current_depth or self.is_terminal or (
        rollout_depth == current_depth + 1 and self.opponent_rollout_policy):
      if self.is_terminal:
      ...
      if new_state_id in self.states:
        ...
        selected_action.after[new_state_id] = State(
            states=self.states,
            game=self.game,
            model=self.model,
            rollout_policy=self.rollout_policy,
            opponent_rollout_policy=self.opponent_rollout_policy)
      selected_action.after[new_state_id].rollout(
          rollout_depth, current_depth + 1, rollout_actions)
